  ...
  ********************************************************/
  async getPersonDetails() {
    try {
      ...
      if (!contactDetail) {
        return handleReject(this.res, HTTP_CODE.NOT_FOUND, HTTP_CODE.NOT_FOUND_CODE, 'Person details not found.');
      }
      ...
      if (contactDetail.isUserDetails) {
        ...
        const isContact = await ContactDirectory.findOne({
          where: {
            ...
            phoneId: this.req.currentUser['PhoneNumber.id']
          }
        });
      ...
      return handleReject(this.res, HTTP_CODE.FAILED, HTTP_CODE.SERVER_ERROR_CODE, 'An error occurred while processing.');
    }
  }
};
