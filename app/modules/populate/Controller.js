...
module.exports = class SeedController extends Controller {
  ...
  async getPhoneNumberForSpam(phoneNumbers, contactNumbers) {
    try {
      ...
      const typeOfNumber = this.getRandomNumberWithProbability(
        TYPE_OF_PHONE_NUMBER.RANDOM_NUMBER,
        TYPE_OF_PHONE_NUMBER.CONTACT_NUMBER,
        TYPE_OF_PHONE_NUMBER.USER_NUMBER
      );
      switch (typeOfNumber) {
        case TYPE_OF_PHONE_NUMBER.RANDOM_NUMBER: // Spam any random number
          const phoneNumber = faker.string.numeric({ length: { min: 7, max: 10 } });
          phoneNumberDetail = await this.registerPhoneNumber(phoneNumber);
          break;
        case TYPE_OF_PHONE_NUMBER.CONTACT_NUMBER: // Spam contact number of any user
          phoneNumberDetail = faker.helpers.arrayElement(contactNumbers)
          break;
        default: // Spam register user's number
